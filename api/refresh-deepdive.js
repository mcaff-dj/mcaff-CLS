// Vercel serverless function: POST /api/refresh-deepdive
//
// Same pattern as /api/refresh.js, but dispatches refresh-deepdive.yml instead of
// refresh.yml - the Deep Dive report (api/_reports/deepdive.html) has its own data
// pipeline (CSAT from MySQL + Agent wise analysis from Drive) and isn't touched by
// the mcaffeine/hyphen refresh at all, so it needs its own workflow + button.
//
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (dispatchResp.status === 204) {
      res.status(200).json({ status: 'started', message: 'Refresh started — this usually takes a few minutes.' });
    } else {
