#!/usr/bin/env python3
"""One-off DDL: adds `ticket_number` to MySQL PEP_CLS.Delivery_escalation - the sheet's own
Ticket Number column (the last column, Z, on both HYPHEN and mCaffeine tabs), missed by
scripts/create_delivery_escalation_table.py's original column list. Plain nullable column, not
part of the (brand, awb_code) unique key - that key is unchanged.
...
DDL = f"ALTER TABLE `{TABLE}` ADD COLUMN `{COLUMN}` VARCHAR(64) NULL AFTER `order_id`"
