#!/usr/bin/env python3
"""One-off: adds calling_agent_process.detractor_lead_type_filter - NPS-Calling's per-agent
process restriction (Delivery / Product / Both), same shape as the existing
detractor_brand_filter column. '' = Both, unrestricted (every existing agent's current behavior).
...
COLUMN = "detractor_lead_type_filter"
DDL = "VARCHAR(16)"
