...
The full (brand, area, score) cross-product is shipped so the frontend (RepeatRateTab.js)
can filter AND re-derive its own "Insights" callout live for whatever brand/area cut the
viewer picks - there's no server-computed "insights" field here, since one baked into this
JSON would only ever reflect the unfiltered view.
