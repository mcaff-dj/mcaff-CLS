"""Pulls yesterday's disposed RTO calling leads from the Postgres lead_assignments
table (the same DB rto-crm.html's admin/auth panel and assign_leads.py use) and
upserts them into PEP_CLS.CLS_RTO_calling in MySQL. Runs daily at 9am IST via
GitHub Actions (see .github/workflows/sync-lead-assignments.yml).
...
same order_id list - never by re-running a date filter, so a row can never be
deleted without this run itself having just confirmed it landed in MySQL. The
retention window matches rto-crm.html's recentAssignments feature (see
api/_lib/db.js), which reads lead_assignments for up to 30 days, so disposed
leads stay visible there for their full lookback window before cleanup.
"""
import os
import sys
...
DELETE_SQL = "DELETE FROM lead_assignments WHERE order_id = ANY(%s)"
...
def delete_rows(order_ids):
    ...
    with psycopg.connect(conn_str) as conn:
        with conn.cursor() as cur:
            cur.execute(DELETE_SQL, (order_ids,))
        conn.commit()
...
def main():
    ...
    print(f"Fetched {len(fresh_rows)} row(s) disposed yesterday (IST) from Postgres lead_assignments.")
    ...
    aged_order_ids = [r[0] for r in aged_rows]
    if aged_order_ids:
        delete_rows(aged_order_ids)
        print(f"Deleted {len(aged_order_ids)} row(s) older than {RETENTION_DAYS} days from Postgres lead_assignments.")
